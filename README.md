# MetroWest SmartHome Configs

Open source Home Assistant, ESPHome, and smart home configurations from [metrowestsmarthome.com](https://metrowestsmarthome.com).

## What's Included

| Folder | Description | Blog Post |
|--------|-------------|-----------|
| `esphome/greenhouse-sensor/` | ESP32-C3 + BME280 temperature/humidity sensor | [Greenhouse Temperature Sensor](https://metrowestsmarthome.com/2025/02/21/greenhouse-temperature-sensor/) |
| `esphome/mini-split-ir/` | Athom IR controller for Fujitsu mini splits | [Making Simple Mini Splits Smarter](https://metrowestsmarthome.com/2025/02/28/making-simple-mini-splits-smarter/) |
| `home-assistant/solar-monitoring/` | MQTT sensors for Renogy solar via BT-1 | [Greenhouse Solar Monitoring](https://metrowestsmarthome.com/2025/06/04/greenhouse-solar-monitoring/) |
| `home-assistant/energy-monitoring/` | rtlamr2mqtt meter reading config | [Monitoring Energy Usage](https://metrowestsmarthome.com/2025/04/09/monitoring-energy-usage-in-home-assistant/) |

## Getting Started

Each folder contains:
- Configuration files with placeholder values (e.g., `YOUR_WIFI_SSID`)
- A README explaining the setup
- Links to the full blog tutorial

### Placeholder Convention

Replace these placeholders with your own values:

| Placeholder | Description |
|-------------|-------------|
| `YOUR_WIFI_SSID` | Your WiFi network name |
| `YOUR_WIFI_PASSWORD` | Your WiFi password |
| `YOUR_API_KEY` | ESPHome API encryption key |
| `YOUR_HA_IP` | Your Home Assistant IP address |
| `YOUR_MQTT_USERNAME` | MQTT broker username |
| `YOUR_MQTT_PASSWORD` | MQTT broker password |

## Support This Project

If these configs help you, consider supporting my work:

- [Buy me a coffee on Ko-fi](https://ko-fi.com/mwsmarthome)
- [Sponsor on GitHub](https://github.com/sponsors/MetroWestSmartHome)
- Star this repo!

## Blog

Visit [metrowestsmarthome.com](https://metrowestsmarthome.com) for full tutorials, guides, and more smart home projects.

## License

MIT License - see [LICENSE](LICENSE) for details.
